 ...
 */
public class POLI extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("POST".equalsIgnoreCase(request.getMethod()))
	    {
			...
			out.println("<body>");
			out.println("<h2> Polindromo con Servlet.</h1>");
			out.println("<h2> ISC 9no. Semestre</h2>");
			out.println("<h2>-----------------------------------------------</h2>");
	    	out.println("<h3>Texto escrito: </h3>" + mystring);
	        //if yes then palindrom, if no then it is not
	        if(Palindrome){
	        	out.println("<h4>Resultado: </h4>");
	           out.println( "<p> Es un polindromo</p>");
	           
	        }else{
	           out.println("<h5>Resultado: </h5>");
	           out.println("<p>NO es polindromo</p>");
	        }
	    }
		}
